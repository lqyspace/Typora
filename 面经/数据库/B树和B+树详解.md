...
最近在学习数据库索引调优的相关知识，数据库系统普遍采用B-/+树作为索引结构（例如MySQL的InnoDB引擎使用的B+树），理解不透而B树，则无法理解数据库的索引机制

另外，B-树，即为B树。因为B树的原因文名称为B-tree，而国内很多人喜欢把B-tree翻译为B-树，其实，这是一个非常不好的直译，很容易让人产生误解。比如很多人会误以为B-树是一种树，而B树又是另外一种树。事实上，B-tree就是指B树，英文为 BalanceTree
...
节点[10] 即为根节点，特征：根节点拥有的子节点的数量上限和内部节点相同，如果根节点不是树中唯一节点的话，至少有两个子节点（不然就变成单支了）。在m阶B树中（根节点非树中唯一节点），那么有关系式：$2<=M<=m$ ，$M$为子节点数量；包含元素的数量$1<=K<=m-1$，$K$为元素数量。
