...
public class ThreadDemo {
    public static void main(String[] args) {
        Thread t = new Thread(()->{
            System.out.println("t当前线程的线程组名称："+Thread.currentThread().getThreadGroup().getName());
            System.out.println("t线程的名称：" +Thread.currentThread().getName());
        });

        t.start();

        System.out.println("执行main方法的线程组："+Thread.currentThread().getThreadGroup().getName());
...
ThreadGroup管理着它下面的Thread，ThreadGroup是一个标椎的向下引用的树状结构，这样设计的原因是 **防止上级线程被下级线程引用而无法有效地被GC回收。**
...
public class ThreadDemo01 extends Thread{
    ...
    public static void main(String[] args) {
        IntStream.range(1,10).forEach(i->{
            Thread t = new Thread(new ThreadDemo01());// Thread本身就继承了Runnable
            t.setPriority(i);
            t.start();
        });
    }
...
总结：1、先声明一个线程组，线程组里面定义 `uncaughtException`方法：该方法只有线程成员抛出unchecked exception时会执行。

2、声明一个线程，将该线程纳入线程组，并抛出一个运行时异常。执行该线程。
