...
- 用户名密码认证通过，会从权限表查处账号拥有的权限与连接关联，之后的权限判断都依赖于此时读取到的权限。
...
在查询优化其中，可以分为 `逻辑查询` 优化阶段和 `物理查询` 优化阶段。
...
截止到现在，还没有真正去读写一个真实的表，仅仅只是产出一个执行计划。就是就进入了 `执行器阶段`。
...
比如，当 `缓冲池`不够用时，需要释放掉一些不常用的页，此时就可以强行采用 `checkpoint` 的方式，将不常用的脏页回写到磁盘上，然后再从缓冲池中将这些页释放掉。这里的脏页指的是缓冲池中被修改过的页，与磁盘上的数据页不一致。
