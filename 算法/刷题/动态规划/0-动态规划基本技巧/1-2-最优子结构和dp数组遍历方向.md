...
可以从子问题的最优解推导出规模更大问题的最优解。

但是，最优子结构并不是动态规划独有的一个性质，能求最值的问题大部分都具有这个性质。但是反过来，**最优子结构性质作为动态规划问题的必要条件，一定是让你求最值**，以后碰见这样的题，思路往动态规划想就对了。

找最优子结构的过程，其实就是证明状态转移方程正确的过程，方程符合最优子结构就可以写暴力解了，写出暴力解就可以看有没有重叠子问题，有则优化，无则OK。
...
看了前文 [动态规划核心套路](https://labuladong.github.io/algo/di-ling-zh-bfe1b/dong-tai-g-1e688/)，我知道了如何一步步优化动态规划问题；

看了前文 [动态规划设计：数学归纳法](https://labuladong.github.io/algo/di-er-zhan-a01c6/dong-tai-g-a223e/dong-tai-g-6ea57/)，我知道了利用数学归纳法写出暴力解（状态转移方程）。

**但就算我写出了暴力解，我很难判断这个解法是否存在重叠子问题**，从而无法确定是否可以运用备忘录等方法去优化算法效率。

对于这个问题，其实我在动态规划系列的文章中写过几次，在这里再统一总结一下吧。

**首先，最简单的方式就是画图，把递归树画出来，看有没有重复的节点。**

比如最简单的例子，[动态规划核心套路](https://labuladong.github.io/algo/di-ling-zh-bfe1b/dong-tai-g-1e688/) 中斐波那契数列的递归树：

![img](https://labuladong.github.io/algo/images/动态规划详解进阶/1.jpg)
...
![img](https://labuladong.github.io/algo/images/最优子结构/2.jpeg)
...
再举个稍微复杂的例子，后文 [正则表达式问题](https://labuladong.github.io/algo/di-er-zhan-a01c6/yong-dong--63ceb/jing-dian--8d516/) 的暴力解代码：
...
比如说后文 [编辑距离问题](https://labuladong.github.io/algo/di-er-zhan-a01c6/zi-xu-lie--6bc09/jing-dian--e5f5e/)，我首先讲的是自顶向下的递归解法，实现了这样一个 `dp` 函数：
...
甚至更让人迷惑的是，有时候发现正向反向遍历都可以得到正确答案，比如我们在 [团灭股票问题](https://labuladong.github.io/algo/di-ling-zh-bfe1b/yi-ge-fang-3b01b/) 中有的地方就正反皆可。
...
![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202308041131075.jpeg)
...
再举一例，回文子序列问题，详见后文 [子序列问题模板](https://labuladong.github.io/algo/di-er-zhan-a01c6/zi-xu-lie--6bc09/dong-tai-g-c481e/)，我们通过过对 `dp` 数组的定义，确定了 base case 处在中间的对角线，`dp[i][j]` 需要从 `dp[i+1][j]`, `dp[i][j-1]`, `dp[i+1][j-1]` 转移而来，想要求的最终答案是 `dp[0][n-1]`，如下图：

![img](https://labuladong.github.io/algo/images/最长回文子序列/4.jpg)

这种情况根据刚才的两个原则，就可以有两种正确的遍历方式：

![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202308041131059.jpeg)
