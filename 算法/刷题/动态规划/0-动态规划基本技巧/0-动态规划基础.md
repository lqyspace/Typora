...
**求解动态规划的核心问题就是穷举。**无穷所有的可行解并不是一件容易的事，只有正确的列举出**状态转移方程**，才能正确的穷举。而且你需要判断问题是否具有**最优子结构**。是否可以**通过原问题得到子问题的解**。另外动态规划问题**存在重叠子问题**，还需要使用**备忘录或DP table**优化穷举。
...
**明确base case——>明确状态（原问题和子问题会变化的量）——>明确选择（引起状态发生改变的量）——>定义`dp`数组/函数的含义——>找到状态转移方程**
