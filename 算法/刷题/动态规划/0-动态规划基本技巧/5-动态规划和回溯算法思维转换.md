...
![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202308051345948.jpeg)
...
![img](https://labuladong.github.io/algo/images/单词拆分/1.jpeg)
...
![img](https://labuladong.github.io/algo/images/单词拆分/2.jpeg)
...
class Solution{
    ...
    boolean backtrace(String s, int i, List<String> wordDict){
        ...
        for(String u : wordDict){
            ...
            if(!sub.equals(u) || !backtrace(s, i+len, wordDict))
                continue;
            ok = true;;
        }
        return ok;
...
![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202308051846570.jpeg)
...
总结：好好理解为什么这个地方是这么用 `备忘录` 的，为什么要将备忘录设置为 `memo[i]`，你的疑虑是什么，你是不是在想，第 `i`位置开始的字符，由于字典中的字符是不确定的，所以从第 `i` 位置开始的字符可能是 false，也可能是 true。如果你是这么想的话，那说明你并没有真正的理解memo的含义以及代码的运行过程，你想呀，从第 `i` 位置开始的字符，去搜索匹配字典中的字符，由于字典中的字符是确定的，所以第 `i` 位置的字符一定是确定的，因为你的代码如果从 `i` 位置开始的字符在字典中存在的，那就一定能匹配上，如果匹配不上，那就说明在字典中一定没有这个字符。所以你的疑虑证明了你对这个过程没有想通透。
...
![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202308051950170.jpeg)
...
class Solution {
    ...
    public void dp(String s, int i, List<String> word){    
        if(i == s.length()){
