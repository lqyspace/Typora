...
具体来说，你需要先阅读并理解前文 [回溯算法核心套路](https://labuladong.github.io/algo/di-ling-zh-bfe1b/hui-su-sua-c26da/)，然后记住如下子集问题和排列问题的回溯树，就可以解决所有排列组合子集相关的问题：

![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202307161016895.jpeg)

![img](https://labuladong.github.io/algo/images/排列组合/2.jpeg)
...
![img](https://labuladong.github.io/algo/images/排列组合/4.jpeg)
...
![img](https://labuladong.github.io/algo/images/排列组合/5.jpeg)
...
![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202307161105298.jpeg)
...
看过前文 [回溯算法核心框架](https://labuladong.github.io/algo/di-ling-zh-bfe1b/hui-su-sua-c26da/) 的读者应该很容易理解这段代码吧，我们使用 `start` 参数控制树枝的生长避免产生重复的子集，用 `track` 记录根节点到每个节点的路径的值，同时在前序位置把每个节点的路径值收集起来，完成回溯树的遍历就收集了所有子集：

![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202307161152011.jpeg)
...
如果你能够生成所有的无重子集，那么你一定可以生成所有的无重组合。

比如说，让你在 `nums=[1,2,3]`中拿2个元素形成所有的组合，你怎么做？

稍微想想就会明白，大小为2的组合，不就是所有大小为2的子集。
...
![img](https://labuladong.github.io/algo/images/排列组合/6.jpeg)
...
**按照之前的思路，画出子集的树形结构，显然，两条值相同的相邻树枝会产生重复：**

![img](https://labuladong.github.io/algo/images/排列组合/8.jpeg)
...
我们可以看到，`[1]`和 `[1, 2]` 这两个结果出现了重复，所以我们需要进行剪枝，如果一个节点有多条值相同的树枝相邻，则只遍历第一条，剩下的都减掉，不要去遍历：

![img](https://labuladong.github.io/algo/images/排列组合/9.jpeg)

**体现在代码上，需要先进行排序，让相同的元素靠在一起，如果发现 `nums[i] == nums[i-1]`，则跳过**：

==**我自己的理解**==：好好看看上面这幅图，理解 `nums[i] == nums[i-1]`的意思，为什么不是 `nums[i] == nums[i+1]`，这样做的目的是为看以前是否访问过相同的值。如果以前访问过相同的值，就没必要继续沿着树枝往深了遍历。而且需要注意的是 `nums[i] == nums[i-1]`的比较是同层的树枝进行比较，而不是上下层进行比较，意思是说同层中如果**遍历过**重复的值，就没必要继续遍历重复的值。所以最终的 `if`循环的条件是 `i>start && nums[i] == nums[i-1]`。
...
class Solution {
    ...
    public void backtrack(int[] nums, boolean[] used, List<Integer> track){
...
![img](https://labuladong.github.io/algo/images/排列组合/12.jpeg)

如果用绿色树枝代表 `backtrack` 函数遍历过的路径，红色树枝代表剪枝逻辑的触发，那么 `!used[i - 1]` 这种剪枝逻辑得到的回溯树长这样：

![img](https://labuladong.github.io/algo/images/排列组合/13.jpeg)

而 `used[i - 1]` 这种剪枝逻辑得到的回溯树如下：

![img](https://labuladong.github.io/algo/images/排列组合/14.jpeg)
...
![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202307181605886.jpeg)
...
看力扣第 39 题「[组合总和open in new window](https://leetcode.cn/problems/combination-sum/)」：
...
![img](https://raw.githubusercontent.com/lqyspace/mypic/master/PicBed/202307181642460.jpeg)
...
![img](https://labuladong.github.io/algo/images/排列组合/10.jpeg)
...
只要从树的角度思考，这些问题看似复杂多变，实则改改 base case 就能解决，这也是为什么我在 [学习算法和数据结构的框架思维](https://labuladong.github.io/algo/di-ling-zh-bfe1b/xue-xi-sua-01220/) 和 [手把手刷二叉树（纲领篇）](https://labuladong.github.io/algo/di-ling-zh-bfe1b/dong-ge-da-334dd/) 中强调树类型题目重要性的原因。

如果你能够看到这里，真得给你鼓掌，相信你以后遇到各种乱七八糟的算法题，也能一眼看透它们的本质，以不变应万变。另外，考虑到篇幅，本文并没有对这些算法进行复杂度的分析，你可以使用我在 [算法时空复杂度分析实用指南](https://labuladong.github.io/algo/di-ling-zh-bfe1b/suan-fa-sh-05f25/) 讲到的复杂度分析方法尝试自己分析它们的复杂度。
